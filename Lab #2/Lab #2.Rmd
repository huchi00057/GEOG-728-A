...
If the dataset contains non-alphanumeric characters or special symbols, the reverse alphabetical order may not produce meaningful results.

-   Categorical vs. Numeric Data:

If you're working with numeric data or other types of categorical data, ordering in reverse alphabetical order might not be relevant or meaningful.
...
Ensure that the data type of the column used for ordering is appropriate for the ordering operation. For example, ordering strings vs. ordering numeric values may have different implications.
...
continent_means <- latest_data %>%
  
  group_by(continent) %>%
  
  summarise(
    
    Mean_LifeExpectancy = mean(lifeExp, na.rm = TRUE),
    
    Mean_Population = mean(pop, na.rm = TRUE),
    
    Mean_GDPperCapita = mean(gdpPercap, na.rm = TRUE)
